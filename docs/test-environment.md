...
- public online survey with browser-local draft restore, demographics followed by one Q4-Q16 question per screen in paper order, a Q16-dependent two-step continuation for contacts and search context, a compact expandable review with consent controls before submit, and a completion screen after successful submit. Both interfaces read Q4-Q16 from the shared paper-question catalog. The review distinguishes required answer processing from optional event invitations, keeps the submit action primary, and places the confirmed full reset below the navigation;
- deliberate `yes`/`no` answers advance immediately unless Q11 opens its war selector; unanswered questions move forward through an explicit `Пропустить` action. Q16 `no` goes directly to review, while Q16 `yes` opens the additional branch. Legacy five-step and previous focused-flow drafts migrate to the corresponding step in the current versioned flow;
- Q16 `yes` first opens a focused required name/phone step, then an optional territory/period/free-text step; changing Q16 to another answer clears only those dependent values and skips both steps. Invitation consent is a separate optional answer from the original paper form and remains independent from Q16. Public phone input accepts familiar formatting but requires 10-15 digits;
- public submission requires explicit consent to processing answers. Invitation consent is optional and is shown independently for every respondent; historical rows keep `Не зафиксировано` rather than being interpreted as refusal;
...
- online drafts expire after 24 hours and never store `contactName` or `contactPhone`; a restored help request returns to the contact step so both values can be entered again;
...
- desktop operator entry keeps one continuous paper-form flow at a restrained maximum width, with explicit gender/age/residence choices, one answered/remaining progress indicator, visual groups, and a contextual next-unanswered action after entry has started;
- phone-width paper entry uses a 15-step guided flow: three explicit demographic choices, one Q4-Q16 question per screen, then a separate final step for the two paper consent marks. The first step cannot be left until each demographic value has been confirmed. Ordinary answers auto-advance, conditional Q11/Q16 details stay on the current step, and back/next controls preserve answers;
...
- q16 help/contact queue with compact status/plan selects, persisted status and operator notes, row list, contact-first inspector, inline editing, recoverable deletion, demo-row generation, fake-only cleanup, and server CSV export use the current filter slice. Deleted questionnaires leave all ordinary slices, charts, counts, and CSV immediately; `Ещё` opens a separate trash screen where the same row can be restored. Real rows have no permanent-delete action. Queue cards prioritize identity, questionnaire date, status, and the next-contact state; long free text stays in the inspector. In `Обращения`, contact and call action come before the workflow, while search context and the full questionnaire stay collapsed until needed. On desktop a new queue slice selects its top-priority row once; an explicit close keeps the inspector closed. Contact and questionnaire lists initially render 20 rows and expose `Показать ещё`; filtering, counters, charts, and export continue to use the full matching slice;
...
- CSV without name/phone remains the visible data-page action. Export with names and phone numbers is a separate confirmed action in the secondary menu and sends `includeContacts=true`; screen masking alone is never treated as the export safety boundary. Contact visibility and demo-data actions also live in the secondary action menu. Rows expose only `Открыть`; editing and deletion stay in the selected-row inspector;
