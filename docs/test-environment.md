...
- q16 help/contact queue with compact status/plan selects, persisted status and operator notes, row list, row inspector with consent state and full Q4-Q16 answer review, inline editing/deletion, demo-row generation, fake-only deletion, and server CSV export use the current filter slice. Contact and questionnaire lists initially render 20 rows and expose `Показать ещё`; filtering, counters, charts, and export continue to use the full matching slice;
