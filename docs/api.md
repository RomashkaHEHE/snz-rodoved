...
- По умолчанию не включает колонки `Имя` и `Номер телефона` и возвращает файл `rodoved-responses-without-name-phone.csv`.
- `includeContacts=true` явно добавляет имя и телефон и меняет имя файла на `rodoved-responses-with-contacts.csv`.
- Обе версии включают отметки согласия и рабочие поля обращения: `Согласие на обработку данных`, `Согласие на приглашения`, `Статус обращения`, `Следующий контакт`, `Заметка по обращению`.
