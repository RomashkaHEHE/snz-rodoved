...
`consentToDataProcessing=true` обязателен для публичной отправки. Если Q16 равен
`yes`, имя и телефон также обязательны. Телефон может содержать пробелы, скобки,
точки и дефисы, но должен содержать от 10 до 15 цифр.
...
`DELETE /api/responses/:id` перемещает активную анкету в корзину. Удалённая
анкета исключается из списков, фильтров, аналитики и CSV.

`GET /api/responses/trash` возвращает удалённые анкеты.

`POST /api/responses/:id/restore` восстанавливает анкету из корзины.
...
- По умолчанию не включает колонки `Имя` и `Номер телефона`.
- `includeContacts=true` включает контактные колонки и используется только
  отдельным подтверждаемым действием интерфейса.
