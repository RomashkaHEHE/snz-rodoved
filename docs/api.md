...
`consentToDataProcessing=true` обязательно для публичной онлайн-отправки. Имя и телефон обязательны только при `q16=yes`; телефон должен содержать от 10 до 15 цифр. `consentToEvents` необязательно.
