...
export function parseFiltersFromQuery(query: unknown): SurveyFilters {
  ...
  const parsed = surveyFiltersSchema.parse({
    ...
    contactNextFrom: parseSingleValue(source.contactNextFrom),
    contactNextTo: parseSingleValue(source.contactNextTo),
    contactNextMissing: parseBooleanValue(source.contactNextMissing),
