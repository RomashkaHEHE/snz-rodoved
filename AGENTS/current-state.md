...
- On phone widths, selecting a `/data` row opens a focused full-viewport detail screen instead of appending the inspector below the list. Contacts stay masked by default, Q4-Q16 are collapsed until requested, editing stays in the same screen, and contact status uses a select; desktop keeps the sticky two-column inspector.
