...
- The test-domain `Обращения` mode is a contact-work queue for q16 help requests. It shows quick status queues (`Все`, `Новое`, `В работе`, `Закрыто`, `Не дозвонились`), sorts open work first, and persists contact status, next-contact date, plus operator notes.
