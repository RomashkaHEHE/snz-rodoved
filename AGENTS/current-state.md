...
- Test-domain public survey starts with demographics, then presents one Q4-Q16 question per screen in paper-form order. The experimental UI consumes the canonical `answerQuestions` catalog from `packages/shared`, so online and paper flows cannot silently diverge in order or wording. `yes`/`no` answers auto-advance except when Q11 or Q16 opens dependent fields; unanswered questions use an explicit `Пропустить` action. Name, phone, territory, period, and free text appear only after a `yes` answer to Q16. The final check uses four collapsed factual summaries, visibly separates required answer processing from optional event invitations, and keeps the confirmed `Начать заново` action below the primary navigation.
