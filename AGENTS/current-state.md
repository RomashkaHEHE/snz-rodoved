...
- Test-domain data screen includes expanded URL-backed filters with removable active-filter chips, demographic/source bars, server CSV export for the current slice, and a protected `Обращения` queue for q16 help requests, including contact phone links, persisted status, operator notes, and contact-status filtering.
