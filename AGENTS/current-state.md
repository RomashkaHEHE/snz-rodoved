...
- Test-domain public survey starts with demographics, then presents one Q4-Q16 question per screen in paper-form order. `yes`/`no` answers auto-advance except when Q11 or Q16 opens dependent fields; unanswered questions use an explicit `Пропустить` action. Name, phone, territory, period, and free text appear only after a `yes` answer to Q16. The final check uses four collapsed factual summaries, visibly separates required answer processing from optional event invitations, and keeps the confirmed `Начать заново` action below the primary navigation.
- Test-domain online draft state has flow version 2. Existing five-step drafts migrate to the equivalent question in the focused flow instead of being discarded.
...
- Test-domain paper entry has a tab-scoped series state. A successful create retains the selected survey date, clears respondent answers, focuses the first field, increments a server-confirmed counter, and records only the confirmed response ID. The mutation response updates local workspace rows directly; the UI does not depend on a second list request to decide whether a write succeeded. `Проверить последнюю` reopens that exact row in the existing editor after save or tab reload. Changing the series date or pressing `Завершить` clears the reference; in-flight submit is disabled to prevent double-tap duplicates.
- Test-domain paper entry also keeps a tab-scoped 24-hour recovery draft for date, demographics, Q4-Q16, Q11 war detail, consent marks, and the mobile step. Contact, search, free-text, and workflow fields are excluded; successful creation or series completion clears the draft.
