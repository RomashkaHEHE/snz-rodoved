...
- Test-domain public survey starts with demographics, then presents one Q4-Q16 question per screen in paper-form order. `yes`/`no` answers auto-advance except when Q11 or Q16 opens dependent fields; unanswered questions use an explicit `Пропустить` action. Name, phone, territory, period, and free text appear only after a `yes` answer to Q16. The final check uses four collapsed factual summaries followed by processing/invitation consent controls and the completion screen.
